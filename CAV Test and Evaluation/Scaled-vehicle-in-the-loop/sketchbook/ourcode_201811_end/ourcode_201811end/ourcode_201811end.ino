...
void Car::outputRCsignal() {
    if (state==0){        //mechine state
    motor.writeMicroseconds( (uint16_t)engine);
    steering.writeMicroseconds( (uint16_t)steer);
    if (car.getRCThrottle()>1575 || car.getRCThrottle()<1425 || car.getRCSteering()>1575 || car.getRCSteering()<1425){ //leave mechine state by human control
    state=1;
    lockmechine=1;
      }
    ...
    delay(1000);
    if (car.getRCThrottle()>1575 || car.getRCThrottle()<1425 || car.getRCSteering()>1575 || car.getRCSteering()<1425){
      state=2;
      locktohuman=1;
    }
